...
public:
    DataSrc(ros::NodeHandle &nh, const string &topic, double freq)
        : src_topic(topic), src_freq(freq), src_timeout(-1.0), stable_stream(false), recv_first_msg(false), recv_msg(false), inform_no_data(true), inform_unstable(true)
    {
        data_sub = nh.subscribe(src_topic, 100, &DataSrc::recvDataCallback, this, ros::TransportHints().tcpNoDelay());
        buf_size = 5 * freq; // 5 seconds buffer

        check_stream_stability_timer = nh.createTimer(ros::Duration(5 / freq), &DataSrc::checkStreamStabilityCallback, this); // Timeout after 5 periods
    }

    DataSrc(ros::NodeHandle &nh, const string &topic, double freq, double timeout)
        : src_topic(topic), src_freq(freq), src_timeout(timeout), stable_stream(false), recv_first_msg(false), recv_msg(false), inform_no_data(true), inform_unstable(true)
    {
        data_sub = nh.subscribe(src_topic, 100, &DataSrc::recvDataCallback, this, ros::TransportHints().tcpNoDelay());
        buf_size = 5 * freq; // 5 seconds buffer
    ...
    void checkStreamStabilityCallback(const ros::TimerEvent &)
    {
        ...
        if (recv_first_msg)
        {
            ...
            if (inform_no_data)
            {
                ROS_ERROR_STREAM("[Odom Fusion] No data from " << src_topic << " yet #^#");
